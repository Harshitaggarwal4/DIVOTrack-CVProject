    ...
    def eval(self):
        ...
        for pred_file in self.tsfiles:
            ...
            if len(df.columns) == 1:
                f = open(pred_file, "r")
                error_message += (
                    "<exc>Submission %s not in correct form. Values in file must be comma separated.<br>Current form:<br>%s<br>%s<br>.........<br><!exc>"
            ...
            if any(count["count"] > 1):
                ...
                error_message += (
                    "<exc> Found duplicate ID/Frame pairs in sequence %s."
                    % pred_file.split("/")[-1]
                )
                for id in doubleIDs:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", type=str, help="Name", default="mvmhat_result")
    parser.add_argument("--type", type=str, help="Type", default="F1")
    ...
    assert args.type in ["F1", "Acc"], "Type must be either 'F1' or 'Acc'"
    ...
    eval.run(
        benchmark_name=benchmark_name, gt_dir=gt_dir, res_dir=res_dir, seq_file=seq_file
    )
